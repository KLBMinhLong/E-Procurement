...
| Department admin | `/admin/departments` mutations | `ADMIN_DEPARTMENT_MANAGE` | Backend đã có facade admin-service gọi IAM-owned department mutations; frontend vẫn có thể dùng `/org/departments` cho read tree |
...
| Controller check | ✅ Có `/admin/config/services*`, `/admin/health`, `/admin/audit-log`, `/admin/audit-log/export`, export job status/download, `/admin/sessions`, `/admin/sessions/{sessionId}/invalidate`, `/admin/catalog/categories`, `/admin/departments`; high-risk config update/restart/rotate-key vẫn chưa có |
...
| Việc | Chi tiết |
|---|---|
| Backend | ✅ `admin-service` public facade gọi IAM-owned internal department mutation APIs; không cross-read DB |
| Contract align | ✅ `/org/departments` giữ read tree; `/admin/departments*` dùng cho create/update/deactivate |
| UI | Extend org chart with create/update/deactivate only if backend supports it |
| Safety | Deactivate must explain active employee/child department constraint |
...
5. Catalog category và department backend đã có; UI category/department admin có thể làm sau sessions.
