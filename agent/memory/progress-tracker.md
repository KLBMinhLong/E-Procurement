...
| E13: Admin & Config Portal (Rest UI/BPMN) | 🔄 | 2026-06-15: `services/admin-service` backend foundation added as Maven/Docker/Compose service on port 8089; implemented `GET /admin/config/services`, `GET /admin/config/services/{serviceName}`, `PUT /admin/config/services/{serviceName}`, `POST /admin/config/services/{serviceName}/restart`, `POST /admin/config/encryption/rotate-key`, `GET /admin/health`, DB-backed `GET /admin/audit-log`, idempotent `POST /admin/audit-log/export`, audit export worker, `GET /admin/audit-log/export/{jobId}`, XLSX download, active session list/invalidate via IAM-owned session store, catalog category admin facade backed by PR-owned category taxonomy, and department create/update/deactivate facade backed by IAM-owned department mutations; guarded by `SYSTEM_CONFIG`/`SYSTEM_AUDIT_VIEW`/`ADMIN_CATALOG_MANAGE`/`ADMIN_DEPARTMENT_MANAGE`, TOTP-confirmed high-risk config actions through IAM internal verification, sanitized immutable audit rows for newly created admin config action requests, accepted session invalidation requests, catalog category mutations, department mutations, and audit export requests, masked sensitive values, service health probes, infra TCP checks, audit Flyway/MyBatis, required time range filters, pagination/export job creation, status polling, local file rendering, no token/session-secret exposure, no admin-service cross-DB ownership leak, and OpenAPI admin contract descriptions document audit side effects/idempotency replay; runtime config executor/secret-manager apply and frontend E13 pages remain |
...
| KPI/report export APIs | ✅ | Async report export API, cycle-time KPI from PR submitted + PO issued projections, SLA KPI with assigned-step denominator, local PDF worker with JasperReports templates, POI-based XLSX workbook export, and type-specific report datasets for all report types. Resolved XML parsing and resource leaks by pre-compiling Jasper templates at build time; decoupled slow IO/rendering processes from database transaction boundaries to prevent connection exhaustion; added font dependencies and headless JVM properties to Docker environment. |
...
| Approval permission routing hardening | ✅ | 2026-06-14: approval-service rule/step model chuyển từ `approverRole` sang `requiredPermission`; IAM internal approver resolver dùng `permission`; approval schema reset/reseed V1-V3; IAM V11/V12 cấp Super Admin approval permissions và cleanup seeded approval grants; frontend approval rule/workflow UI dùng permission code; PR detail load approval process từ approval-service; `mvn -pl services/approval-service test` và `npm run build` pass |
