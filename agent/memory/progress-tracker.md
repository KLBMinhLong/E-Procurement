...
| E13: Admin & Config Portal (Rest UI/BPMN) | 🔄 | 2026-06-15: `services/admin-service` backend foundation added as Maven/Docker/Compose service on port 8089; implemented `GET /admin/config/services`, `GET /admin/config/services/{serviceName}`, `GET /admin/health`, DB-backed `GET /admin/audit-log`, idempotent `POST /admin/audit-log/export`, audit export worker, `GET /admin/audit-log/export/{jobId}`, XLSX download, active session list/invalidate via IAM-owned session store, catalog category admin facade backed by PR-owned category taxonomy, and department create/update/deactivate facade backed by IAM-owned department mutations; guarded by `SYSTEM_CONFIG`/`SYSTEM_AUDIT_VIEW`/`ADMIN_CATALOG_MANAGE`/`ADMIN_DEPARTMENT_MANAGE`, masked sensitive values, service health probes, infra TCP checks, audit Flyway/MyBatis, required time range filters, pagination/export job creation, status polling, local file rendering, no token/session-secret exposure, and no admin-service cross-DB ownership leak; high-risk config mutation and frontend E13 pages remain |
