...
public class UpdateCatalogCategoryUseCase {
    ...
    private static final String IDEMPOTENCY_OPERATION = "update-catalog-category";
    ...
    public CatalogCategoryAdminView execute(ManageCatalogCategoryCommand command, String idempotencyKey) {
        ...
        if (cached.isPresent()) {
            log.info("[ACTION] Idempotency hit UpdateCatalogCategory | actorId={} | key={}",
        ...
        validate(command);
        if (!catalogCategoryRepository.existsActiveByCode(command.code())) {
        ...
        CatalogCategory category = CatalogCategory.reconstitute(
                command.code(),
                command.name(),
                command.parentCode(),
                ...
                now,
                false);
        catalogCategoryRepository.update(category, command.actorId());
