...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleValidation(
        ...
        List<ValidationError> details = exception.getBindingResult().getFieldErrors().stream()
                ...
                .toList();
        log.warn("[EXCEPTION][{}] Validation failed | path={}", ErrorCode.VAL_001.code(), request.getRequestURI());
        return ResponseEntity.status(ErrorCode.VAL_001.status())
                .body(ApiResponse.failure(
                        ErrorCode.VAL_001.code(),
                        ErrorCode.VAL_001.message(),
                        details,
                        RequestIdUtil.resolve(request)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleTypeMismatch(
            ...
            HttpServletRequest request) {
        log.warn("[EXCEPTION][{}] Type mismatch | path={}", ErrorCode.VAL_001.code(), request.getRequestURI());
        return ResponseEntity.status(ErrorCode.VAL_001.status())
                .body(ApiResponse.failure(
                        ErrorCode.VAL_001.code(),
                        ErrorCode.VAL_001.message(),
    ...
    public ResponseEntity<ApiResponse<Void>> handleSecurity(Exception exception, HttpServletRequest request) {
        ...
        return ResponseEntity.status(ErrorCode.IAM_004.status())
                .body(ApiResponse.failure(
                        ErrorCode.IAM_004.code(),
                        ErrorCode.IAM_004.message(),
                        null,
                        RequestIdUtil.resolve(request)));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleUnexpected(Exception exception, HttpServletRequest request) {
        log.error("[EXCEPTION][{}] Unexpected failure | path={}",
                ErrorCode.SYS_001.code(),
                request.getRequestURI(),
                exception);
        return ResponseEntity.status(ErrorCode.SYS_001.status())
                .body(ApiResponse.failure(
                        ErrorCode.SYS_001.code(),
                        ErrorCode.SYS_001.message(),
                        null,
                        RequestIdUtil.resolve(request)));
    }
